...
public class ImageConvert {
    ...
    public static void main(String[] args) {
        ...
        String dir = "D:\\张豹\\shuiyin\\res";

        System.out.println("输入转换后的存储目录：");

        String saveDir = "D:\\张豹\\shuiyin\\tar";
    ...
    public static void replaceImageColor(String file, String dstFile, Color srcColor, Color targetColor) throws IOException {
        ...
        for (int i = 0; i < bi.getWidth(); i++) {
            for (int j = 0; j < bi.getHeight(); j++) {
                ...
                //锁定水印范围
                if(i>bi.getWidth()/2-100 && i<bi.getWidth()/2+100 && j>bi.getHeight()-100){
                    int color = bi.getRGB(bi.getWidth()/2-100, bi.getHeight()-100);
                    Color oriColor = new Color(color);
                    int red = oriColor.getRed();
                    int greed = oriColor.getGreen();
                    int blue = oriColor.getBlue();
                    bi.setRGB(i, j, oriColor.getRGB());
                }
            }
        }
